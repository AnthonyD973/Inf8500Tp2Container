...
# Install packages
RUN mkdir -p /etc/bash_completion.d && apt-get update && apt-get install -y bash-completion && apt-get upgrade -y && apt-get install -y apt-file bzip2 libbz2-dev wget cmake make gzip gdb perl git nano g++-7 g++-4.8 zip unzip zlib1g-dev htop && apt-file update
